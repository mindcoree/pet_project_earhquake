...
Ниже приведен базовый SQL-скрипт для создания схем и таблиц DWH. Он соответствует текущей структуре проекта: `stg` для сырого слоя, `ods` для очищенных событий и `dm` для витрины (dimensions + fact).
